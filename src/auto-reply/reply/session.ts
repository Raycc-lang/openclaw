...
export async function initSessionState(params: {
  ...
  // Use mtime-guarded cache for session identity resolution.  The original
  // `skipCache: true` was introduced for correctness (#17971) but causes a
  // full synchronous readFileSync + JSON.parse on *every* inbound message,
  // which regresses latency on low-resource hosts as sessions.json grows.
  //
  // `mtimeGuard: true` re-reads only when the file's mtime has actually
  // changed, preserving correctness while avoiding redundant disk I/O for
  // burst/back-to-back messages that arrive before any write occurs.
