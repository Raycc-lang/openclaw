...
type LoadSessionStoreOptions = {
  skipCache?: boolean;
  /**
   * When true, bypass the TTL-based cache but still return the cached store if
   * the file's mtime has not changed since the last load.  This gives
   * correctness comparable to `skipCache` (no stale data from TTL window) while
   * avoiding the cost of a full readFileSync + JSON.parse when the underlying
   * file hasn't been written to.  Useful on the hot inbound-message path where
   * burst messages arrive faster than any session-store write.
   */
  mtimeGuard?: boolean;
};
...
export function loadSessionStore(
  ...
  // mtimeGuard: skip the TTL window check but still return the cached copy
  // when the file hasn't been modified.  This avoids the expensive
  // readFileSync + JSON.parse on every inbound message while keeping data
  // fresh (we re-read as soon as any writer touches the file).
  if (opts.mtimeGuard) {
    const cached = SESSION_STORE_CACHE.get(storePath);
    if (cached) {
      const snap = getFileStatSnapshot(storePath);
      // Compare both mtime and size to catch writes that land within the
      // same mtime granularity window (common on Windows / fast bursts).
      if (
        snap != null &&
        snap.mtimeMs === cached.mtimeMs &&
        snap.size === cached.sizeBytes
      ) {
        return structuredClone(cached.store);
      }
      // mtime or size changed (or file disappeared) — fall through to full read.
      invalidateSessionStoreCache(storePath);
    }
    // No cached entry yet — fall through to full read (result will be cached
    // below as long as skipCache is not also set).
  } else if (!opts.skipCache && isSessionStoreCacheEnabled()) {
    // Standard TTL-based cache path.
    const cached = SESSION_STORE_CACHE.get(storePath);
  ...
  let statSnap = getFileStatSnapshot(storePath);
  ...
  for (let attempt = 0; attempt < maxReadAttempts; attempt++) {
    try {
      ...
      // Re-stat after successful read so the cached snapshot reflects the
      // file state we actually parsed (not a pre-read snapshot).
      statSnap = getFileStatSnapshot(storePath) ?? statSnap;
      break;
    } catch {
  ...
  // Cache the result if caching is enabled (or mtimeGuard is active).
  if (opts.mtimeGuard || (!opts.skipCache && isSessionStoreCacheEnabled())) {
    SESSION_STORE_CACHE.set(storePath, {
      store: structuredClone(store), // Store a copy to prevent external mutations
      loadedAt: Date.now(),
      storePath,
      mtimeMs: statSnap?.mtimeMs,
      sizeBytes: statSnap?.size,
    });
  }
